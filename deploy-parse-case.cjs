// Скрипт деплоя Edge Function parse-case через Supabase Management API
const fs = require('fs');
...
async function deployFunction() {
  if (!ACCESS_TOKEN) {
    ...
    console.log('2. Создайте новый токен (Personal Access Token)');
    console.log('3. Выполните: set SUPABASE_ACCESS_TOKEN=your-token');
    console.log('4. Запустите этот скрипт: node deploy-parse-case.js');
    process.exit(1);
  }
