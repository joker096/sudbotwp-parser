      ...
      <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex gap-2 px-6 py-3 border-b border-slate-200 dark:border-slate-700">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'documents' ? 'bg-accent text-white' : 'text-slate-600 dark:text-slate-300 hover:text-slate-900'}`}
          >
            Документы
          </button>
          <button
            onClick={() => setActiveTab('templates')}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'templates' ? 'bg-accent text-white' : 'text-slate-600 dark:text-slate-300 hover:text-slate-900'}`}
          >
            ...
            <div className="grid grid-cols-2 gap-2">
              {TEMPLATES.map((template) => (
                <button
                  ...
                  className="flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-800 hover:bg-blue-50 hover:bg-blue-900/20 rounded-xl text-left transition-colors"
                >
